...
namespace DryRunner
{
  ...
  public class TestSiteManager
  {
    ...
    /// <summary>
    /// Changes the configuration of the web application and then restarts it (so that the new configuration comes into effect).
    /// </summary>
    public void ChangeConfigurationAndRestart(Func<XDocument, XDocument> changeConfiguration)
    {
      _server.Stop();

      _deployer.ChangeConfiguration(changeConfiguration);
